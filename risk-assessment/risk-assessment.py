            ...
            if sent_data['ResponseMetadata']['HTTPStatusCode'] != 200:
                logging.exception(
                    'Failed to upload image {} to bucket {}'.format(
                        computed_image_key,
                        bucket_base_name + '-processed'
                    )
                )
            update_images_processed(
                computed_image_key, model_version, result['label']
            )
...
def anonymize(img, img_name):
    ...
    new_img_name = (f'{prefix}_XXXXXXXX_{get_study_id(patient_id)}'
                    f'_XXXX-XX-XX_{suffix}')
...
def update_images_processed(image_name, model_version, label):
    do_sql(('INSERT INTO images_processed(time,name,model,label) '
            f'SELECT CURRENT_TIMESTAMP(), "{image_name}",'
            f'"{model_version}","{label.split(",")[0]}";'))


def update_images_anonymized(image_name):
    do_sql(('INSERT INTO images_anonymized(time,name) '
            f'SELECT CURRENT_TIMESTAMP(), "{image_name}";'))
